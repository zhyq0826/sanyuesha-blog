---
title: "SQL中的where条件，在数据库中提取与应用浅析"
summary: "> 此文是阿里数据库内核研发专家何登成的文章 http://hedengcheng.com/?p=577 比较详细的记录了 SQL 语句解析 where 的过程，对于理解 where 中的条件命中 index 有帮助，可以直接去原文，评论区有一些有价值的讨论"
date: "2018-05-18"
tags: 
  - "sql"
group: "mysql"
...
针对上面的SQL，应用这个提取规则，提取出来的 Index First Key 为 `(b >= 2, c > 1)`。由于 c 的条件为 `>`，提取结束，不包括 d。
...
针对上面的SQL，应用这个提取规则，提取出来的 Index Last Key 为 `(b < 8)`，由于是 `<` 符号，因此提取 b 之后结束。
