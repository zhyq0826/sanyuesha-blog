...
function normalizePost(entry: CollectionEntry<'posts'>) {
  const published = entry.data.published ?? entry.data.date
...
export async function getSortedPosts() {
  ...
  const normalized = allPosts.map((post) => normalizePost(post))
